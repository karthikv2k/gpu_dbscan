...
class GpuDbscan():
    def __init__(self, X, index_type="CPU", gpu_device_num=0):
        self.X = X
        self.knn_max_k = 0
        self.knn_search_cnt = 0
        self.gpu_index = self.get_gpu_index(X)
        self.cpu_index = self.get_cpu_index(X)
    ...
    def knn_search_single_instance_gpu(self, point, eps, start_k=512, end_k=512 * 4):
        ...
        if len(I) >= start_k:
            if start_k <= end_k:
                return self.knn_search_single_instance_cpu(self, point, eps)
            else:
...
if __name__ == '__main__':
    X = utils.get_test_blobs()
    gd = GpuDbscan(X)
    cluster_ids, core_point_flag, visited_flag = gd.gpu_dbscan(0.2, 10)
